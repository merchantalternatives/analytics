...
function getUrlStats(shortUrl) {
    const clicks = clickstreamData.filter(click => click['Short link'] === shortUrl);
...
function renderUrlCards() {
    ...
    filteredLinks = filteredLinks.map(link => ({
        ...link,
        stats: getUrlStats(link.short_url)
    }));
...
function createUrlCard(link) {
    ...
    card.addEventListener('click', () => showUrlDetail(link));
    
    return card;
}

function showUrlDetail(link) {
    ...
    const clicks = clickstreamData.filter(click => click['Short link'] === link.short_url);
...
function updatePagination(totalClicks) {
    ...
    if (currentPage > 1) {
        ...
        prevBtn.onclick = () => {
            currentPage--;
            updateClicksTable(clickstreamData.filter(click => click['Short link'] === selectedUrl));
        };
